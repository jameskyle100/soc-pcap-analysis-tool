...
def extract_http(pkt: Any, layers: dict[str, Any]) -> dict[str, Any]:
    if not pkt.haslayer(layers["Raw"]):
        return {}
    raw = bytes(pkt[layers["Raw"]].load)
    data = safe_decode(raw[:4096])
    if not data:
        return {}
...
def packet_to_record(pkt: Any, layers: dict[str, Any]) -> PacketRecord:
    ...
    if pkt.haslayer(layers["Raw"]):
        raw_payload = bytes(pkt[layers["Raw"]].load)
        payload_length = len(raw_payload)
        payload_preview = safe_decode(raw_payload[:512])[:160]
    tcp_flags = None
    tcp_seq = None
    tcp_ack = None
    if pkt.haslayer(layers["TCP"]):
